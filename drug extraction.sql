...
WHERE   domain_id = 'Drug' AND
        ...
        e.person_id = 62074985 AND
        LOWER(route_source_value) LIKE '%iv%'
GROUP BY e.person_id, concept_id, concept_name, LOWER(dose_unit_source_value), e.drug_exposure_start_datetime, e.drug_exposure_end_datetime, route_source_value
HAVING ("sum"(quantity) <> 0)
ORDER BY sum_quantity DESC
LIMIT 100
